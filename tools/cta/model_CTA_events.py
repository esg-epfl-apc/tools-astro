...
Texp = 50.0  # http://odahub.io/ontology#TimeIntervalHours
...
hdul = fits.open(
    "IRFS/fits/Prod5-North-20deg-AverageAz-4LSTs09MSTs.18000s-v0.1.fits.gz"
)
...
hdul[0].header["CRPIX1"] = Npix / 2.0
hdul[0].header["CRPIX2"] = Npix / 2.0
...
migra_axis = MapAxis.from_bounds(
    -2, 2, nbin=150, node_type="edges", name="migra"
)
...
len(energy_bins), float(np.min(energy_bins) / u.TeV), float(
    np.max(energy_bins) / u.TeV
)
...
_oda_outs = []
_oda_outs.append(
    ("out_model_CTA_events_theta2", "theta2_galaxy.output", theta2)
)
_oda_outs.append(
    ("out_model_CTA_events_spectrum", "spectrum_galaxy.output", spectrum)
)
...
_simple_outs.append(
    (
        "out_model_CTA_events_events_fits",
