...
def intrinsic_dFdE(E):
    # return PowerLaw(normalization=Norm / (u.TeV * u.cm**2 * u.s),
    #                index=index, e_ref=0.25*u.TeV)(E)
    EE = (E).to(u.GeV).value
    return Assumed(EE) / (u.TeV * u.cm**2 * u.s)
...
#                     a=-2, b =-0.05,
#                     e_ref=0.1*u.TeV)(E)
intrinsic_dFdE(1.0 * u.TeV)

# After EBL absorption:
def dFdE(E):
    EE = (E).to(u.TeV).value
    tau_interp = np.interp(EE, ee, tau)
    atten = exp(-tau_interp)
    return intrinsic_dFdE(E) * atten
