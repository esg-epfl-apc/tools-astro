...
date = ""  # oda:String; oda:description "Observation date (YYYY-MM-DDThh:mm:ss or MET or filename)"
keyword = "TSTART"  # oda:String,oda:optional; oda:description "Name of date/time keyword when 'date' is a file"
detmask = "NONE"  # oda:POSIXPath,oda:optional; oda:description "Name of input quality map (or NONE)"
outcaldbmask = ""  # oda:String,oda:optional; oda:description "Name of CALDB file found by batdetmask (set upon return)"
outdetmask = ""  # oda:String,oda:optional; oda:description "Detector mask file and extension actually used (set upon return)"
...
if "C_data_product_" in inp_dic.keys():
    ...
    inp_pdic = inp_dic
date = str(inp_pdic["date"])

keyword = (
    str(inp_pdic["keyword"])
    if inp_pdic.get("keyword", None) is not None
    else None
)
...
outdetmask = (
    str(inp_pdic["outdetmask"])
    if inp_pdic.get("outdetmask", None) is not None
    else None
)
...
command = "batdetmask"
params_names = "infile,date,outfile,keyword,detmask,outcaldbmask,outdetmask,chatter,clobber,history,mode"
...
_galaxy_meta_data = {}
_oda_outs = []
_oda_outs.append(("out_batdetmask_result", "result_galaxy.output", result))
