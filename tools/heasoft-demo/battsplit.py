...
infile = "auxfile.fits"  # oda:POSIXPath; oda:description "Input auxiliary file name"
gtifile = "NONE"  # oda:POSIXPath,oda:optional; oda:description "Name of good time interval file (or NONE)"
tolerance = 5.0  # oda:Float,oda:optional ; oda:lower_limit 0; oda:description "Amount of 'difference' which triggers a split"
expr = "SQRT((IMX-IMX{-1})^2 + (IMY-IMY{-1})^2)/#deg"  # oda:String,oda:optional; oda:description "Expression to compute 'difference' between two rows"
diffcol = "_DIFF"  # oda:String,oda:optional; oda:description "Name of 'difference' column in temporary file"
...
tolerance = (
    float(inp_pdic["tolerance"])
    if inp_pdic.get("tolerance", None) is not None
    else None
)
...
diffcol = (
    str(inp_pdic["diffcol"])
    if inp_pdic.get("diffcol", None) is not None
    else None
)
...
command = "battsplit"
params_names = "infile,infile,outfile,gtifile,tolerance,expr,diffcol,cleanup,clobber,chatter,history,mode"
...
_galaxy_meta_data = {}
_oda_outs = []
_oda_outs.append(("out_battsplit_result", "result_galaxy.output", result))
