...
infile = "spectrum.pha"  # oda:POSIXPath; oda:description "Input event PHA file name"
outroot = "response"  # oda:nan; oda:description "Output response matrix file name root"
hkfile = "NONE"  # oda:POSIXPath,oda:optional; oda:description "DAP housekeeping file name (or NONE)"
opts = "NONE"  # oda:String,oda:optional; oda:description "Extra options to pass to batdrmgen"
rows = "-"  # oda:String,oda:optional; oda:description "Row numbers in SPECTRUM extension of PHA fits file"
...
outroot = str(inp_pdic["outroot"])

hkfile = (
    str(inp_pdic["hkfile"])
    if inp_pdic.get("hkfile", None) is not None
    else None
)

opts = (
    str(inp_pdic["opts"]) if inp_pdic.get("opts", None) is not None else None
)
...
command = "batdrmgen-multi"
params_names = (
    "infile,infile,outroot,hkfile,opts,rows,clobber,chatter,history,mode"
)
optional_file_params = "hkfile"
...
_oda_outs = []
_oda_outs.append(
    ("out_batdrmgen_multi_result", "result_galaxy.output", result)
)
