...
infile = "NONE"  # oda:POSIXPath; oda:description "Input file name"
rows = 1  # oda:Integer; oda:description "row number of cube to be flattened ?"
levels = (
    "-"  # oda:String; oda:description "Level range(s) used in flattening?"
)
...
rows = int(inp_pdic["rows"])
levels = str(inp_pdic["levels"])
...
command = "batdph2dpi"
params_names = "infile,infile,outfile,rows,levels,clobber,chatter,history,mode"
...
_galaxy_meta_data = {}
_oda_outs = []
_oda_outs.append(("out_batdph2dpi_result", "result_galaxy.output", result))
