...
infile = "NONE"  # oda:POSIXPath; oda:description "Input image"
rows = "-"  # oda:String,oda:optional; oda:description "Images to process (or '-' for all)"
distfile = "CALDB"  # oda:POSIXPath,oda:optional; oda:description "Name of the distortion map file (or CALDB)"
outcoord = "TRUE"  # oda:String,oda:optional ; oda:allowed_value "TRUE","APPARENT"; oda:description "Output coordinate system (TRUE or APPARENT)"
copyall = True  # oda:Boolean,oda:optional; oda:description "Copy all extensions to output?"
...
rows = (
    str(inp_pdic["rows"]) if inp_pdic.get("rows", None) is not None else None
)

distfile = (
    str(inp_pdic["distfile"])
    if inp_pdic.get("distfile", None) is not None
    else None
)

outcoord = (
    str(inp_pdic["outcoord"])
    if inp_pdic.get("outcoord", None) is not None
    else None
)

copyall = (
    bool(inp_pdic["copyall"])
    if inp_pdic.get("copyall", None) is not None
    else None
)
...
command = "batwarpimg"
params_names = "infile,infile,outfile,rows,distfile,outcoord,copyall,clobber,chatter,history,mode"
optional_file_params = "distfile"
...
_galaxy_meta_data = {}
_oda_outs = []
_oda_outs.append(("out_batwarpimg_result", "result_galaxy.output", result))
