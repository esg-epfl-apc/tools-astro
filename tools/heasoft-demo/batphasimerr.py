...
infile = "NONE"  # oda:POSIXPath; oda:description "Input spectrum filename"
exposure = 0.0  # oda:Float ; oda:lower_limit 0.0; oda:description "Exposure time of spectrum"
bkgfile = (
    "CALDB"  # oda:POSIXPath; oda:description "BAT background rate file name"
)
eboundfile = "NONE"  # oda:POSIXPath; oda:description "File containing input spectrum binning"
pcodefr = 1.0  # oda:Float,oda:optional ; oda:lower_limit 0.00001 ; oda:upper_limit 1.00; oda:description "Partial coding fraction"
alpha = 0.28  # oda:Float,oda:optional ; oda:lower_limit 0.0; oda:description "BAT mask weighting correction factor"
...
exposure = float(inp_pdic["exposure"])
bkgfile = str(inp_pdic["bkgfile"])
eboundfile = str(inp_pdic["eboundfile"])

pcodefr = (
    float(inp_pdic["pcodefr"])
    if inp_pdic.get("pcodefr", None) is not None
    else None
)

alpha = (
    float(inp_pdic["alpha"])
    if inp_pdic.get("alpha", None) is not None
    else None
)
...
command = "batphasimerr"
params_names = "infile,infile,exposure,bkgfile,eboundfile,pcodefr,alpha,chatter,history,mode"
...
_galaxy_meta_data = {}
_oda_outs = []
_oda_outs.append(("out_batphasimerr_result", "result_galaxy.output", result))
