...
def mc_rotate(mc_file, jet_half_size_deg, jet_direction_deg, psf_deg=180, rotated_file=None):
    if rotated_file is None:
        rotated_file = f'{mc_file}_{jet_half_size_deg}_{jet_direction_deg}_{psf_deg}'

    jet_half_size = np.longdouble(jet_half_size_deg) / 180. * math.pi  # jet openning angle
    jet_direction = np.longdouble(jet_direction_deg) / 180. * math.pi  # angle between direction to source and jet axes
    ...
    jet_axes = np.array([0, np.sin(jet_direction), np.cos(jet_direction)], dtype=np.longdouble)
        ...
        for line in lines:
            ...
            if columns[0] == '#':
                if 'weight' in columns:
                    column_names = columns[1:3] + ["Theta", "Phi", "dT_raw/T", "dT_calc/T"] + columns[9:]
                ...
                r = (particle[2:5])
                dz = r[
                    2]  # dz is roughly equal to time delay in units of full path (assuming very small deflection angle)

                # negative dz may apper in data as a result of rounding errors
                dz = max(dz, 0)  # replacing negative values by zeros

                r[2] = 1. - dz  # now r is position of particle
                ...
                if dr2 > 0:
                    ...
                    toCenter = np.array([[cos + ux * ux * (1 - cos), ux * uy * (1 - cos), uy * sin],
                                         [ux * uy * (1 - cos), cos + uy * uy * (1 - cos), -ux * sin],
                                         [-uy * sin, ux * sin, cos]])

                    zAxes = np.array([0., 0., 1.], dtype=np.longdouble)
                    # test
                    ...
                    nInitial *= (1. / norm_nInitial)  # normalize after rotation to avoid rounding errors
                    # calculating angle between starting direction and jet axis
                    startCos = np.dot(nInitial, jet_axes)
                    startAngle = 0
                    if startCos < 1:  # avoid roundout errors if angle is very close to zero
                    ...
                    if startAngle < jet_half_size:  # if the particle is inside the cone (passes cut)
                        # calculate angle between arrival direction and direction to source
                        cosBeta = np.dot(r, n)  # this angle is invariant (doesn't depend on turns)
                        if cosBeta < 1:  # avoid rounding errors if angle is very close to zero
                            beta = np.arccos(cosBeta)
                            nFinal = np.dot(toCenter, n).transpose()
                            cosPhi = nFinal[0] / np.sin(
                                beta)  # second angle (could be useful in assymetric tasks) if beta=0 it is undefined (could be any)
                            if cosPhi < 1 and cosPhi > -1:
                                phi = np.arccos(cosPhi)  # returns value in interval [0,pi]
                                if nFinal[1] < 0:
                                    phi = 2 * math.pi - phi
                        ...
                        continue
                elif jet_direction > jet_half_size:  # no deflection: check if jet is pointing towards observer
                    continue

                if beta * 180.0 / math.pi > psf_deg:
                    continue

                output = [particle[0], particle[1], beta * 180.0 / math.pi, phi * 180.0 / math.pi, dz, t] + list(
                    particle[8:])
