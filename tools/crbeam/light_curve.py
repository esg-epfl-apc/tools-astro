...
default_params = {'weight_col': 2,
                  ...
                  'show': False,
                  'add_alpha': 0.,  # multiply weights be E_src^{-add_alpha}
...
def light_curve(delay, weights, **kwargs):
    ...
    while True:
        xmin = 0.5 * (delay[bin_idx - 1] + delay[bin_idx]) if bin_idx > 0 else delay[bin_idx]
        if xmin > max_t:
            break
...
def make_plot(path, label='verbose', **kwargs):
    ...
    max_t = params['max_t']
    out_name = f'{path}_f{col + 1}_E{Emin:{fmt}}-{Emax:{fmt}}TeV_th{psf}_r{max_t}{suffix}.'
    ...
    if max_t < 0:
        median, max_t = weighted_quantile(delay, [0.5, -0.01 * max_t], sample_weight=weights)
    else:
...
if __name__ == "__main__":
    def usage_exit(reason=''):
        ...
        print("usage: python", sys.argv[0], 'data_file', *['{}={}'.format(k, v) for k, v in default_params.items()],
              file=sys.stderr)
        exit(1)
